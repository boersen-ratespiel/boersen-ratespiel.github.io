...
**Social-Media-Vorschau:** `og-image.jpg` (1200×630, per Canvas gezeichnet und mit `sips` als JPEG statt PNG exportiert — deutlich kleinere Datei bei gleicher Bildqualität für einen Gradient-Hintergrund) wird über Open-Graph-/Twitter-Card-Meta-Tags in `index.html` eingebunden, damit geteilte Links (WhatsApp, Twitter/X, etc.) mit Bild + Beschreibung statt nacktem Link angezeigt werden. `og:image`/`og:url` sind auf die GitHub-Pages-URL (`https://boersen-ratespiel.github.io/`) hartkodiert — falls die Seite unter einer anderen URL landet, müssen diese beiden Tags angepasst werden. Das Repo liegt seit 2026-09-16 unter der Organisation `boersen-ratespiel` (nicht mehr unter dem persönlichen Account `WM-Predit`), extra dafür angelegt, damit GitHub Pages als kurze User-Site-URL (`boersen-ratespiel.github.io`, ohne `/boersen-ratespiel/`-Unterordner) ausliefert statt der längeren Projekt-Site-URL.
