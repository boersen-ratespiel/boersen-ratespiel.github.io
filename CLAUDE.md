...
**Social-Media-Vorschau:** `og-image.jpg` (1200×630, per Canvas gezeichnet und mit `sips` als JPEG statt PNG exportiert — deutlich kleinere Datei bei gleicher Bildqualität für einen Gradient-Hintergrund) wird über Open-Graph-/Twitter-Card-Meta-Tags in `index.html` eingebunden, damit geteilte Links (WhatsApp, Twitter/X, etc.) mit Bild + Beschreibung statt nacktem Link angezeigt werden. `og:image`/`og:url` sind auf die erwartete GitHub-Pages-URL (`https://wm-predit.github.io/boersen-ratespiel/`) hartkodiert — falls die Seite unter einer anderen URL landet, müssen diese beiden Tags angepasst werden.
